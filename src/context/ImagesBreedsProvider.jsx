...
const ImagesBreedsProvider = ({ children }) => {
  ...
  const getImagesBreedsBySubBreed = async (info) => {
    try {
      const url = `https://dog.ceo/api/breed/${info.breed}/${info.subBreed}/images`;
  ...
  return (
    <ImagesBreedsContext.Provider
      value={{ getImagesBreeds, getImagesBreedsBySubBreed, imagesBreeds }}
    >
      {children}
    </ImagesBreedsContext.Provider>
  );
